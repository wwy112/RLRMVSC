...
% result ACC-accuracy  MIhat-nmi AMGLF-F AMGLP-P AMGLR-R AMGLFnmi-nmi
% AMGLFavgent-avgent AMGLFAR-AR
if size(Y,2) ~= 1
    Y = Y';
end;
if size(predY,2) ~= 1
...
% F P R AR
[AMGLF AMGLP AMGLR] = compute_f(Y,res);
[AMGLFA AMGLFnmi AMGLFavgent] = compute_nmi(Y,res);
[AMGLFAR AMGLFRI AMGLFMI AMGLFHI] = RandIndex(Y,res);
result = [ACC MIhat AMGLF AMGLP AMGLR AMGLFnmi AMGLFavgent AMGLFAR];
